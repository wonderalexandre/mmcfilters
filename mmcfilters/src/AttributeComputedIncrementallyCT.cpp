...
ImageFloatPtr AttributeComputedIncrementallyCT::computerAttributeMapping(ComponentTree* tree, Attribute attribute) {
    auto [attrNames, buffer] = AttributeComputedIncrementallyCT::computeSingleAttribute(tree, attribute);
...
std::pair<std::shared_ptr<AttributeNames>, std::shared_ptr<float[]>> AttributeComputedIncrementallyCT::computeAttributesByComputer(ComponentTree* tree, std::shared_ptr<AttributeComputerCT> comp, const DependencyMap& availableDeps) {
...
std::pair<std::shared_ptr<AttributeNames>, std::shared_ptr<float[]>> AttributeComputedIncrementallyCT::computeSingleAttribute(ComponentTree* tree, AttributeOrGroup attrOrGroup, const DependencyMap& availableDeps) {
    ...
    auto comp = AttributeFactoryCT::create(attrOrGroup);
    DependencyMap available = availableDeps;
...
std::pair<std::shared_ptr<AttributeNamesWithDelta>, std::shared_ptr<float[]>> AttributeComputedIncrementallyCT::computeSingleAttributeWithDelta(ComponentTree* tree, Attribute attribute, int delta, std::string padding, const DependencyMap& availableDeps) {
	/*
		Valores de padding:
	...
	*/
    ...
    auto [attributeNamesBase, attrsBase] = AttributeComputedIncrementallyCT::computeSingleAttribute(tree, attribute, availableDeps);
...
static std::vector<std::shared_ptr<AttributeComputerCT>> getOrderedComputers(const std::vector<AttributeOrGroup>& attrOrGroups) {
    using ACptr = std::shared_ptr<AttributeComputerCT>;
    ...
    std::function<void(Attribute)> collect = [&](Attribute attr) {
        ...
        auto comp = AttributeFactoryCT::create(attr);
        const auto& ref = *comp;
        ...
        for (const auto& depOrGroup : comp->requiredAttributes()) {
            ...
            for (const auto& depAttr : deps) {
                collect(depAttr);
                auto depComp = AttributeFactoryCT::create(depAttr);
...
std::pair<std::shared_ptr<AttributeNames>, std::shared_ptr<float[]>> AttributeComputedIncrementallyCT::computeAttributes(ComponentTree* tree, const std::vector<AttributeOrGroup>& attributes, const DependencyMap& providedDependencies) {
...
std::shared_ptr<ContoursCT> AttributeComputedIncrementallyCT::extractCompactContours(ComponentTree* tree){
    std::shared_ptr<ContoursCT> contoursMT = std::make_shared<ContoursCT>(tree);
...
std::vector<std::unordered_set<int>> AttributeComputedIncrementallyCT::extractNonCompactContours(ComponentTree* tree){
