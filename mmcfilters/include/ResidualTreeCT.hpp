...
class ResidualTreeCT;
using ResidualTreeCTPtr = std::shared_ptr<ResidualTreeCT>;
class ResidualTreeCT{

    protected:
      NodeResCTPtr root;
      std::shared_ptr<AttributeOpeningPrimitivesFamilyCT> primitivesFamily;
      ComponentTree* tree;
      ImageUInt8Ptr maxContrastLUT;
      std::shared_ptr<int[]> associatedIndexesLUT;
      int numNodes;
      ImageUInt8Ptr restOfImage;
      std::vector<NodeResCTPtr> nodes;

    public:


    ResidualTreeCT(std::shared_ptr<AttributeOpeningPrimitivesFamilyCT> primitivesFamily) {
    ...
    void createTree(){
      ...
      this->root = std::make_shared<NodeResCT>(tree, InvalidNode, this->numNodes++, isDesirableResidue);
      for (NodeId nodeMaxCriterion : nodesWithMaximumCriterium){
        ...
        for (NodeId  currentNode: tree->getNodesOfSubtree( nodeMaxCriterion  )){
          NodeResCTPtr parent = this->nodes[tree->getParentById(currentNode)];
          if (this->primitivesFamily->isSelectedForPruning(currentNode)){ // first node in Nr(i)
            ...
            this->nodes[currentNode] = std::make_shared<NodeResCT>(tree, currentNode, this->numNodes++, isDesirableResidue);
    ...
    NodeResCTPtr getRoot(){
      return this->root;
    }

    NodeResCTPtr getNodeRes(NodeId  node){
    ...
    void computerMaximumResidues(){
      ...
      std::stack<NodeResCTPtr> s;
      s.push( this->root );
      while(!s.empty()){
        NodeResCTPtr nodeRes = s.top(); s.pop();
        ...
        for (NodeResCTPtr child : nodeRes->getChildren()){
          s.push( child ); 
        }
      }
    ...
    */    
    ...
    ImageUInt8Ptr filtering(std::vector<bool> criterion){
      std::stack<NodeResCTPtr> s;
      for (NodeResCTPtr node : this->root->getChildren()){
        s.push(node);
      }
      ...
      while (!s.empty()){
        ...
        for (NodeResCTPtr child : node->getChildren()){
          s.push(child);
        }
      }
    ...
    /*std::list<NodeResCTPtr> getListNodes(){
    ...
    ImageInt32Ptr getAssociatedImage(){
      ...
      for (int pidx = 0; pidx < size; pidx++){
        (*out)[pidx] = this->associatedIndexesLUT[this->tree->getSC(pidx)];
      }
      return out;
    }
