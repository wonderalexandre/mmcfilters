...
private:
    ...
    static AdjacencyRelation makeAdjacency(const WeightedMorphologicalTree<std::uint8_t>& minTree, const WeightedMorphologicalTree<std::uint8_t>& maxTree) {
...
public:
    DualMinMaxTreeIncrementalFilterPybind(
        std::shared_ptr<WeightedMorphologicalTree<std::uint8_t>> minTree,
    ...
    std::shared_ptr<WeightedMorphologicalTree<std::uint8_t>> minTree() const {
        return minTree_;
    }

    std::shared_ptr<WeightedMorphologicalTree<std::uint8_t>> maxTree() const {
        return maxTree_;
    }
};
...
domain and must carry adjacency metadata.)doc")
        .def(py::init<
                std::shared_ptr<WeightedMorphologicalTree<std::uint8_t>>,
                std::shared_ptr<WeightedMorphologicalTree<std::uint8_t>>>(),
            "minTree"_a,
            "maxTree"_a,
            "Create the paired incremental updater.")
        ...
        .def_property_readonly("minTree", &DualMinMaxTreeIncrementalFilterPybind::minTree,
            "Borrowed min-tree owner updated by paired pruning operations.")
        .def_property_readonly("maxTree", &DualMinMaxTreeIncrementalFilterPybind::maxTree,
            "Borrowed max-tree owner updated by paired pruning operations.");
...
`filter` are interpreted in increasing order for the selected CASF attribute.)doc")
        ...
        .def("filter", [](adjust::CasfComponentTrees<std::uint8_t>& self, const std::vector<double>& thresholds) {
            return PybindUtils::toNumpy(self.filter(thresholds));
        }, "thresholds"_a,
            "Run the connected alternating sequential filter and return a 2D uint8 image.")
        .def_property_readonly("minTree", [](adjust::CasfComponentTrees<std::uint8_t>& self) -> const WeightedMorphologicalTree<std::uint8_t>& {
            return self.minTree();
        }, py::return_value_policy::reference_internal,
            "Internal min-tree maintained by the CASF.")
        .def_property_readonly("maxTree", [](adjust::CasfComponentTrees<std::uint8_t>& self) -> const WeightedMorphologicalTree<std::uint8_t>& {
            return self.maxTree();
        }, py::return_value_policy::reference_internal,
            "Internal max-tree maintained by the CASF.")
        .def_property_readonly("attribute", &adjust::CasfComponentTrees<std::uint8_t>::attribute,
            "CASF attribute used to evaluate thresholds.")
