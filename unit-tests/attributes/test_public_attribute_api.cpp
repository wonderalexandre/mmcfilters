...
int main() {
    ...
    auto weightedAttributes = AttributeComputation::computeAttributes(weighted, std::vector<AttributeOrGroup>{AREA, LEVEL, MAX_DIST});
    requireEqual(weightedAttributes.first.NUM_ATTRIBUTES, 3, "weighted public attribute stride");
    require(weightedAttributes.second.size() ==
                static_cast<std::size_t>(weighted.topology().getNumInternalNodeSlots()) * static_cast<std::size_t>(weightedAttributes.first.NUM_ATTRIBUTES),
            "weighted public attribute buffer shape");
    requireEqual(weightedAttributes.second[weightedAttributes.first.linearIndex(weighted.topology().getRoot(), AREA)], 16.0f,
                 "root AREA through public weighted facade");

    auto topologyAttributes =
        AttributeComputation::computeTopologyAttributes(weighted.topology(), std::vector<AttributeOrGroup>{AREA, BOX_WIDTH, BALANCE_NODE});
    requireEqual(topologyAttributes.first.NUM_ATTRIBUTES, 3, "topology public attribute stride");
    requireEqual(topologyAttributes.second[topologyAttributes.first.linearIndex(weighted.topology().getRoot(), AREA)], 16.0f,
                 "root AREA through explicit topology facade");

    auto deltaLevel = AttributeComputation::computeSingleAttributeWithDelta(weighted, LEVEL, AltitudeDiff<std::uint8_t>{1}, 1);
    requireEqual(deltaLevel.first.NUM_ATTRIBUTES, 3, "delta public attribute stride");
    require(deltaLevel.second.size() ==
                static_cast<std::size_t>(weighted.topology().getNumInternalNodeSlots()) * static_cast<std::size_t>(deltaLevel.first.NUM_ATTRIBUTES),
            "delta public attribute buffer shape");

    const NodeId root = weighted.topology().getRoot();
    requireEqual(deltaLevel.second[deltaLevel.first.linearIndex(root, LEVEL, 0)], static_cast<float>(weighted.getAltitude(root)),
                 "delta center LEVEL through public weighted facade");

    auto meanOnly = AttributeComputation::computeAttributes(weighted, std::vector<AttributeOrGroup>{MEAN_LEVEL});
    ...
    auto eccentricityOnly = AttributeComputation::computeAttributes(weighted, std::vector<AttributeOrGroup>{ECCENTRICITY});
    ...
    requireHiddenDependenciesAreAbsent(eccentricityOnly.first, {AREA, CENTRAL_MOMENT_20, CENTRAL_MOMENT_02, CENTRAL_MOMENT_11},
    ...
    auto topologyEccentricityOnly = AttributeComputation::computeTopologyAttributes(weighted.topology(), std::vector<AttributeOrGroup>{ECCENTRICITY});
    ...
    requireHiddenDependenciesAreAbsent(topologyEccentricityOnly.first, {AREA, CENTRAL_MOMENT_20, CENTRAL_MOMENT_02, CENTRAL_MOMENT_11},
                                       "ECCENTRICITY topology request");
    requireFiniteLiveNodeValues(weighted.topology(), topologyEccentricityOnly.first, topologyEccentricityOnly.second, ECCENTRICITY,
                                "ECCENTRICITY topology request");

    auto huOnly = AttributeComputation::computeAttributes(weighted, std::vector<AttributeOrGroup>{HU_MOMENT_1});
    ...
    auto rectangularityOnly = AttributeComputation::computeTopologyAttributes(weighted.topology(), std::vector<AttributeOrGroup>{RECTANGULARITY});
    ...
    requireFiniteLiveNodeValues(weighted.topology(), rectangularityOnly.first, rectangularityOnly.second, RECTANGULARITY, "RECTANGULARITY topology request");

    auto grayGroup = AttributeComputation::computeAttributes(weighted, std::vector<AttributeOrGroup>{AttributeGroup::GRAY_LEVEL});
