...
template<class T>
concept HasExecuteWithMSER = requires(T& uao) {
    uao.executeWithMSER(1, 1);
};

template<class T, class MinTree, class MaxTree>
concept CanBuildSdrtFromSeeds =
    AltitudeValue<T> &&
    requires(
        sdrt::SelfDualResidualTreeBuilder<T>& builder,
        ImagePtr<T> image,
        MinTree&& minTree,
        MaxTree&& maxTree) {
        builder.build(image, std::forward<MinTree>(minTree), std::forward<MaxTree>(maxTree));
    };

static_assert(std::is_same_v<
    decltype(MorphologicalTreeFactory::createMaxTree(std::declval<ImageUInt8Ptr>(), 1.5)),
    WeightedMorphologicalTree<std::uint8_t>>);
static_assert(std::is_same_v<
    decltype(MorphologicalTreeFactory::createMinTree(std::declval<ImageUInt8Ptr>(), 1.5)),
    WeightedMorphologicalTree<std::uint8_t>>);
static_assert(std::is_same_v<
    decltype(MorphologicalTreeFactory::createTreeOfShapes(
        std::declval<ImageUInt8Ptr>(),
        ToSInterpolation::SelfDual,
        ToSDefaultInfinityRow,
        ToSDefaultInfinityCol)),
    WeightedMorphologicalTree<std::uint8_t>>);
static_assert(std::is_same_v<
    decltype(MorphologicalTreeFactory::createSelfDualResidualTree(std::declval<ImageUInt8Ptr>(), 1.5)),
    WeightedMorphologicalTree<std::uint8_t>>);
static_assert(std::is_same_v<
...
static_assert(std::is_same_v<
    decltype(std::declval<adjust::CasfComponentTrees<std::uint8_t>&>().minTree()),
    const WeightedMorphologicalTree<std::uint8_t>&>);
static_assert(std::is_same_v<
    decltype(std::declval<adjust::CasfComponentTrees<std::uint8_t>&>().maxTree()),
    const WeightedMorphologicalTree<std::uint8_t>&>);
static_assert(std::is_same_v<
    decltype(std::declval<adjust::CasfComponentTrees<std::int32_t>&>().minTree()),
    const WeightedMorphologicalTree<std::int32_t>&>);
static_assert(std::is_same_v<
    decltype(std::declval<adjust::CasfComponentTrees<float>&>().filter(std::declval<const std::vector<double>&>())),
    ImageFloatPtr>);

static_assert(std::is_constructible_v<
    adjust::DualMinMaxTreeIncrementalFilter<std::uint8_t>,
    WeightedMorphologicalTree<std::uint8_t>*,
    WeightedMorphologicalTree<std::uint8_t>*,
    AdjacencyRelation&>);
static_assert(!std::is_constructible_v<
    adjust::DualMinMaxTreeIncrementalFilter<std::uint8_t>,
    WeightedTreeView<std::uint8_t>*,
    WeightedTreeView<std::uint8_t>*,
    AdjacencyRelation&>);
static_assert(std::is_constructible_v<
    adjust::DualMinMaxTreeIncrementalFilter<std::int32_t>,
    WeightedMorphologicalTree<std::int32_t>*,
    WeightedMorphologicalTree<std::int32_t>*,
    AdjacencyRelation&>);
static_assert(!std::is_constructible_v<
    ...
    AdjacencyRelation&>);
static_assert(std::is_constructible_v<
    adjust::DualMinMaxTreeIncrementalFilterLeaf<std::uint8_t>,
    WeightedMorphologicalTree<std::uint8_t>*,
    WeightedMorphologicalTree<std::uint8_t>*,
    AdjacencyRelation&>);
static_assert(!std::is_constructible_v<
    ...
    AdjacencyRelation&>);
static_assert(std::is_constructible_v<
    adjust::DualMinMaxTreeIncrementalFilterLeaf<std::int32_t>,
    WeightedMorphologicalTree<std::int32_t>*,
    WeightedMorphologicalTree<std::int32_t>*,
    AdjacencyRelation&>);
static_assert(!std::is_constructible_v<
...
static_assert(CanBuildSdrtFromSeeds<std::uint8_t, WeightedMorphologicalTree<std::uint8_t>, WeightedMorphologicalTree<std::uint8_t>>);
static_assert(CanBuildSdrtFromSeeds<std::int32_t, WeightedMorphologicalTree<std::int32_t>, WeightedMorphologicalTree<std::int32_t>>);
static_assert(!CanBuildSdrtFromSeeds<std::uint8_t, WeightedTreeView<std::uint8_t>&, WeightedTreeView<std::uint8_t>&>);
static_assert(!CanBuildSdrtFromSeeds<std::int32_t, WeightedMorphologicalTree<std::uint8_t>, WeightedMorphologicalTree<std::uint8_t>>);
...
int main() {
    ...
    auto adjacency = *minTree.topology().getAdjacencyRelation();
    ...
    requireEqual(
        intUao.getMaxContrastImage()->getSize(),
        intImage->getSize(),
        "typed int32 UAO MSER output must match image size");
    ...
    requireEqual(
        floatUao.getMaxContrastImage()->getSize(),
        floatImage->getSize(),
        "typed float UAO MSER output must match image size");
    ...
    require(casf.minTree().topology().getTreeType() == MorphologicalTreeKind::MIN_TREE, "CASF min-tree accessor must expose the owner state");
    require(casf.maxTree().topology().getTreeType() == MorphologicalTreeKind::MAX_TREE, "CASF max-tree accessor must expose the owner state");

    sdrt::SelfDualResidualTreeBuilder<std::uint8_t> builder;
    builder.build(image, std::move(minTree), std::move(maxTree));
    require(builder.getRoot() != InvalidNode, "SDRT builder must materialize owner-backed state");
