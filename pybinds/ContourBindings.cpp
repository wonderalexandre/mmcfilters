...
void initContoursComputedIncrementally(py::module_& m) {
    ...
    py::class_<ContourRange>(m, "ContourRange", py::module_local(false),
        "Lazy iterable range of row-major pixel indices for one internal-node contour.")
        ...
        .def("empty", &ContourRange::empty,
            "Return true when the contour has no pixels.");
    ...
    struct ContoursIterator {
        ...
        explicit ContoursIterator(Contours& self)
    ...
    py::class_<ContoursIterator>(m, "ContoursIterator", py::module_local(false),
        "Iterator over `(node_id, ContourRange)` pairs for all live nodes.")
...
may materialize and cache its subtree contour; later reads use cached storage.)doc")
        ...
        .def("getContour", &Contours::getContour,
            "nodeId"_a,
            "Return a lazy contour range for one live internal node.")
        .def("materializeAll", &Contours::materializeAll,
            "Materialize and cache contours for every live node.")
        .def_property_readonly("isMaterialized", &Contours::isMaterialized,
            "Whether all live-node contours have been materialized.")
        .def("isContourMaterialized", &Contours::isContourMaterialized,
            "nodeId"_a,
            "Return whether one live node contour has been materialized.");
    ...
    py::class_<ContoursComputedIncrementallyPybind>(m, "ContourComputation", py::module_local(false),
        ...
        .def_static("extraction", py::overload_cast<MorphologicalTreePybindPtr>(&ContoursComputedIncrementallyPybind::extraction),
            "tree"_a,
            "Extract compact contours from an unweighted topology object.")
        .def_static("extraction", py::overload_cast<std::shared_ptr<WeightedMorphologicalTree<std::uint8_t>>>(&ContoursComputedIncrementallyPybind::extraction),
            "tree"_a,
            "Extract compact contours from a weighted tree.");
}
