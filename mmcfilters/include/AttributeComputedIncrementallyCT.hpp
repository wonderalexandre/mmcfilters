...
public:
 ...
 /*
    ...
    static void computerAttribute(ComponentTree* tree, NodeId root, 
        ...
        preProcessing(root);
        for (NodeId child : tree->getChildrenById(root)) {
            AttributeComputedIncrementallyCT::computerAttribute(tree, child, preProcessing, mergeProcessing, postProcessing); // passar por ref (sem cópia)
	...
	static std::shared_ptr<ContoursCT> extractCompactContours(ComponentTree* tree);
    static std::shared_ptr<ContoursCT> extractCompactContours(ComponentTreePtr tree){ return extractCompactContours(tree.get()); }
    ...
    static std::pair<std::shared_ptr<AttributeNames>, std::shared_ptr<float[]>> computeAttributesByComputer(ComponentTreePtr tree, std::shared_ptr<AttributeComputerCT> comp, const DependencyMap& available = {}){ return computeAttributesByComputer(tree.get(), comp, available); }
...
class ContoursCT{
    ...
    ContoursCT(ComponentTreePtr tree): ContoursCT(tree.get()) {}
