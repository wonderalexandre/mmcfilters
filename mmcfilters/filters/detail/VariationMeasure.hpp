 ...
 */
inline void validateStabilityNeighborhoodShape(
    const MorphologicalTree& tree,
    const std::vector<NodeId>& ascendants,
...
[[nodiscard]] inline bool isFiniteVariation(Real value) {
...
[[nodiscard]] inline Real computeVariationValue(
    NodeId node,
    const std::vector<NodeId>& ascendants,
    const std::vector<NodeId>& descendants,
    AttrGetter& attrAt) {
 ...
 */
template<std::floating_point Real, class AttrGetter>
[[nodiscard]] inline std::vector<Real> computeVariationsFromNeighborhood(
    const MorphologicalTree& tree,
    const std::vector<NodeId>& ascendants,
    const std::vector<NodeId>& descendants,
    AttrGetter& attrAt) {
    ...
    for (NodeId nodeId : tree.getAliveNodeIds()) {
        variation[static_cast<std::size_t>(nodeId)] =
            computeVariationValue<Real>(nodeId, ascendants, descendants, attrAt);
    }
    return variation;
}
 ...
 */
template<std::floating_point Real>
[[nodiscard]] inline bool isStrictVariationMinimum(
    NodeId node,
    const std::vector<Real>& variation,
    const std::vector<NodeId>& ascendants,
    const std::vector<NodeId>& descendants) {
    const NodeId ascendant = ascendants[static_cast<std::size_t>(node)];
    const NodeId descendant = descendants[static_cast<std::size_t>(node)];
    ...
    return isFiniteVariation(center) &&
           isFiniteVariation(ascendantValue) &&
           isFiniteVariation(descendantValue) &&
           center < ascendantValue &&
 ...
 */
template<std::floating_point Real, class AttrGetter>
[[nodiscard]] inline std::vector<uint8_t> selectStrictVariationMinima(
    const MorphologicalTree& tree,
    const std::vector<Real>& variation,
    const std::vector<NodeId>& ascendants,
    const std::vector<NodeId>& descendants,
    ...
    for (NodeId nodeId : tree.getAliveNodeIds()) {
        ...
        if (variation[static_cast<std::size_t>(nodeId)] < maxVariation &&
            attribute >= minAttr &&
            attribute <= maxAttr) {
 ...
 */
template<std::floating_point Real>
[[nodiscard]] inline NodeId nodeWithMinimumVariationInWindow(
    NodeId node,
    const std::vector<Real>& variation,
    const std::vector<NodeId>& ascendants,
    const std::vector<NodeId>& descendants) {
    ...
    const NodeId candidates[3] = {
        node,
        descendants[static_cast<std::size_t>(node)],
        ascendants[static_cast<std::size_t>(node)]};
    ...
    for (const NodeId candidate : candidates) {
        ...
        if (bestNode == InvalidNode || value < bestValue ||
            (value == bestValue && candidate < bestNode)) {
            bestNode = candidate;
            bestValue = value;
        }
    }
