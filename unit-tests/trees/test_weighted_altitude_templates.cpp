...
static_assert(std::is_same_v<
    decltype(MorphologicalTreeFactory::createMaxTree(std::declval<ImageInt32Ptr>(), 1.5)),
    WeightedMorphologicalTree<std::int32_t>>);
static_assert(std::is_same_v<
    decltype(MorphologicalTreeFactory::createMinTree(std::declval<ImageFloatPtr>(), 1.5)),
    WeightedMorphologicalTree<float>>);
static_assert(std::is_same_v<
    decltype(MorphologicalTreeFactory::createFromHigraParent<double>(
        std::declval<std::span<const NodeId>>(),
        std::declval<std::span<const double>>(),
        4,
        4,
        MorphologicalTreeKind::MAX_TREE,
        std::declval<std::optional<AdjacencyRelation>>())),
    WeightedMorphologicalTree<double>>);
static_assert(std::is_same_v<
    decltype(std::declval<const WeightedMorphologicalTree<std::int32_t>&>().reconstructionImage()),
    ImageInt32Ptr>);
static_assert(std::is_same_v<
    decltype(std::declval<const WeightedMorphologicalTree<float>&>().reconstructionImage()),
    ImageFloatPtr>);
static_assert(std::is_same_v<
    decltype(AttributeComputation::projectNodeValuesToExportedHigra(
...
template<class T>
void checkGenericStaticAltitudeAccess(const WeightedMorphologicalTree<std::uint8_t>& weighted, NodeId sampleNodeId) {
    const auto& tree = weighted.topology();
    ...
    require(
        TreeAltitudeAlgorithms::getAltitude(view, sampleNodeId) == altitude[static_cast<std::size_t>(sampleNodeId)],
        "templated getAltitude must preserve the altitude value type");

    requireThrows<std::runtime_error>(
        [&]() {
            const std::vector<T> wrongSize(
                static_cast<std::size_t>(tree.getNumInternalNodeSlots() - 1),
                T{});
    ...
    requireThrows<std::invalid_argument>(
        [&]() {
            static_cast<void>(TreeAltitudeAlgorithms::getAltitude(view, InvalidNode));
        },
        "templated getAltitude must reject invalid node ids");
}

template<class T>
void checkWeightedTreeViewContract(const WeightedMorphologicalTree<std::uint8_t>& weighted, NodeId sampleNodeId, const std::string& label) {
    const auto& tree = weighted.topology();
    const std::vector<T> altitude = makeGenericAltitude<T>(weighted);
    ...
    requireEqual(
        static_cast<int>(view.altitude().size()),
        tree.getNumInternalNodeSlots(),
        label + " view altitude span size");
    requireEqual(
        static_cast<int>(inferredSpanView.altitude().size()),
        tree.getNumInternalNodeSlots(),
        label + " inferred span view altitude span size");
    requireEqual(
        static_cast<int>(inferredBufferView.altitude().size()),
        tree.getNumInternalNodeSlots(),
        label + " inferred buffer view altitude span size");
    require(
        view.getAltitude(sampleNodeId) == altitude[static_cast<std::size_t>(sampleNodeId)],
        label + " view getAltitude must preserve the altitude value type");
    require(
        inferredSpanView.getAltitude(sampleNodeId) == altitude[static_cast<std::size_t>(sampleNodeId)],
        label + " inferred span view getAltitude must preserve the altitude value type");
    require(
        inferredBufferView.getAltitude(sampleNodeId) == altitude[static_cast<std::size_t>(sampleNodeId)],
    ...
    if constexpr (std::is_floating_point_v<T>) {
        requireNear(
            view.getNodeResidue(sampleNodeId),
            expectedResidue,
            static_cast<T>(1.0e-6),
            label + " view residue must match static generic helper");
    } else {
        requireEqual(
            view.getNodeResidue(sampleNodeId),
            expectedResidue,
    ...
    const auto fromView = AttributeComputation::computeAttributesFromAltitudeSpan(
        view,
        {LEVEL, GRAY_HEIGHT});
    const auto fromSpan = AttributeComputation::computeAttributesFromAltitudeSpan(
        inferredSpanView,
        {LEVEL, GRAY_HEIGHT});
    for (NodeId nodeId : tree.getAliveNodeIds()) {
        for (Attribute attribute : {LEVEL, GRAY_HEIGHT}) {
            requireNear(
                fromView.second[fromView.first.linearIndex(nodeId, attribute)],
                fromSpan.second[fromSpan.first.linearIndex(nodeId, attribute)],
                1.0e-5f,
    ...
    requireThrows<std::invalid_argument>(
        [&]() {
            static_cast<void>(view.getAltitude(InvalidNode));
        },
        label + " view must reject invalid node ids");
}
    ...
    const std::string& label)
{
    for (NodeId nodeId : tree.getAliveNodeIds()) {
        for (Attribute attribute : attributes) {
            requireNear(
                actual.second[actual.first.linearIndex(nodeId, attribute)],
                expected.second[expected.first.linearIndex(nodeId, attribute)],
                1.0e-5f,
    ...
    const std::string& label)
{
    for (NodeId nodeId : tree.getAliveNodeIds()) {
        for (int d = -delta; d <= delta; ++d) {
            requireNear(
                actual.second[actual.first.linearIndex(nodeId, attribute, d)],
                expected.second[expected.first.linearIndex(nodeId, attribute, d)],
                1.0e-5f,
                label + " node " + std::to_string(nodeId) + " delta " + std::to_string(d));
        }
    }
}

void requireImageNear(
    const ImageFloatPtr& actual,
    const ImageFloatPtr& expected,
    const std::string& label)
{
...
template<class T>
ImagePtr<T> makeTypedComponentTreeFixture() {
...
template<class T>
ImagePtr<T> makeTypedComponentTreeFixtureWithValue(std::size_t index, T value) {
...
template<class T>
void requireTypedOwnerMatchesCanonicalImageTree(
    const WeightedMorphologicalTree<T>& typed,
    const WeightedMorphologicalTree<std::uint8_t>& canonical,
    const ImageUInt8Ptr& expectedReconstruction,
    const std::string& label)
{
    requireEqual(
        typed.topology().getNumInternalNodeSlots(),
        canonical.topology().getNumInternalNodeSlots(),
        label + " internal node slot count");
    requireEqual(
        typed.topology().getNumTotalProperParts(),
        canonical.topology().getNumTotalProperParts(),
        label + " proper part count");
    requireEqual(
        typed.topology().getRoot(),
        canonical.topology().getRoot(),
        label + " root");
    requireEqual(
        static_cast<int>(typed.getAltitudeBuffer().size()),
        typed.topology().getNumInternalNodeSlots(),
        label + " typed altitude buffer size");

    for (NodeId nodeId : typed.topology().getAliveNodeIds()) {
        requireNear(
            static_cast<double>(typed.getAltitude(nodeId)),
            static_cast<double>(canonical.getAltitude(nodeId)),
            1.0e-6,
            label + " node altitude");
    }
    ...
    requireVectorEqual(
        collectImageValues(typed.reconstructionImage()),
        expectedTypedReconstruction,
        label + " typed reconstruction");
}
...
void checkTypedHigraImportFactory(ImageUInt8Ptr image) {
    ...
    auto importedFloat = MorphologicalTreeFactory::createFromHigraParent<float>(
        ...
        std::span<const float>(floatHigraAltitude),
        image->getNumRows(),
        image->getNumCols(),
        MorphologicalTreeKind::MAX_TREE,
    ...
    for (std::size_t i = 0; i < roundtripAltitude.size(); ++i) {
        requireNear(
            roundtripAltitude[i],
            floatHigraAltitude[i],
            1.0e-6f,
            "typed Higra factory altitude roundtrip");
    }
    ...
    requireVectorEqual(
        collectImageValues(importedFloat.reconstructionImage()),
        expectedFloatReconstruction,
        "typed Higra factory reconstruction");
    ...
    requireThrows<std::invalid_argument>(
        [&]() {
            ...
            static_cast<void>(MorphologicalTreeFactory::createFromHigraParent<double>(
                std::span<const NodeId>(higraParent),
                std::span<const double>(wrongAltitude),
                image->getNumRows(),
                image->getNumCols(),
                MorphologicalTreeKind::MAX_TREE,
                AdjacencyRelation(image->getNumRows(), image->getNumCols(), 1.5)));
...
void checkFiniteFloatAltitudeValidation(ImageUInt8Ptr image) {
    requireThrows<std::invalid_argument>(
        [&]() {
            static_cast<void>(MorphologicalTreeFactory::createMaxTree(
                makeTypedComponentTreeFixtureWithValue<float>(3, std::numeric_limits<float>::quiet_NaN()),
                1.5));
        },
    ...
    requireThrows<std::invalid_argument>(
        [&]() {
            static_cast<void>(MorphologicalTreeFactory::createMinTree(
                makeTypedComponentTreeFixtureWithValue<float>(4, std::numeric_limits<float>::infinity()),
                1.5));
        },
    ...
    requireThrows<std::invalid_argument>(
        [&]() {
            static_cast<void>(MorphologicalTreeFactory::createMaxTree(
                makeTypedComponentTreeFixtureWithValue<float>(5, -std::numeric_limits<float>::infinity()),
                1.5));
        },
    ...
    requireThrows<std::invalid_argument>(
        [&]() {
            floatMax.setAltitudeBuffer(std::move(invalidBuffer));
        },
        "WeightedMorphologicalTree<float>::setAltitudeBuffer must reject NaN");

    requireThrows<std::invalid_argument>(
        [&]() {
            floatMax.setAltitude(sampleNodeId, std::numeric_limits<float>::infinity());
        },
...

    {
        ...
        requireThrows<std::invalid_argument>(
            [&]() {
                static_cast<void>(editor.createDetachedNode(-std::numeric_limits<float>::infinity()));
            },
...

    {
        ...
        requireThrows<std::invalid_argument>(
            [&]() {
                editor.setNodeAltitude(sampleNodeId, std::numeric_limits<float>::quiet_NaN());
            },
...
void checkImageFactoriesDeduceAltitudeType(ImageUInt8Ptr image) {
    ...
    requireTypedOwnerMatchesCanonicalImageTree(
        int32Max,
        canonicalMax,
        image,
        "Image<int32_t> max-tree factory");
    ...
    requireTypedOwnerMatchesCanonicalImageTree(
        ...
        canonicalMin,
        image,
        "Image<int32_t> min-tree factory");
    {
        const std::vector<AttributeOrGroup> requests{
        ...
        requireComputedAttributesEquivalent(
            typedAttrs,
            canonicalAttrs,
            int32Max.topology(),
            expectedAttrs,
        ...
        requireImageNear(
            AttributeComputation::computeAttributeMapping(int32Max, CONTOUR_PERIMETER),
        ...
        "Image<float> min-tree factory");
    {
        ...
        requireComputedAttributesEquivalent(
            singleLevel,
            canonicalSingleLevel,
            floatMin.topology(),
            {LEVEL},
            "Image<float> owner single-attribute overload must compute LEVEL");

        requireImageNear(
            AttributeComputation::computeAttributeMapping(floatMin, LEVEL),
            AttributeComputation::computeAttributeMapping(canonicalMin, LEVEL),
            "Image<float> owner mapping overload must support LEVEL");
    }
}

template<class T>
void checkTypedOwnerExportedHigraAttributeProjection(const std::string& label) {
    auto image = makeTypedComponentTreeFixture<T>();
    ...
    const auto expectedSize =
        static_cast<std::size_t>(typed.topology().getNumTotalProperParts() + typed.topology().getNumNodes()) *
    ...
    const auto [sampleRow, sampleCol] = ImageUtils::to2D(sampleProperPart, typed.topology().getNumColsOfImage());
    const float sampleAltitude = static_cast<float>(typed.getAltitude(sampleOwner));
    auto projectedAt = [&](Attribute attribute) {
        return projected[computed.first.linearIndex(sampleProperPart, attribute)];
    };
...
void checkWeightedTreeViewIncrementalAttributePipeline(const WeightedMorphologicalTree<std::uint8_t>& weighted) {
    ...
    requireImageNear(
        AttributeComputation::computeAttributeMapping(ownerView, LEVEL),
        AttributeComputation::computeAttributeMapping(weighted, LEVEL),
    ...
    requireComputedAttributesNear(
        externalRegular,
        externalFromAltitudeSpan,
        tree,
        {LEVEL, GRAY_HEIGHT},
    ...
    requireComputedAttributesNear(
        ...
        tree,
        {MAX_DIST},
        "external view attribute-pipeline path must support MAX_DIST");
    ...
    requireImageNear(
        AttributeComputation::computeAttributeMapping(externalView, LEVEL),
        AttributeComputation::computeAttributeMapping(weighted, LEVEL),
...
void checkFloatResidueAndMonotonicity(const WeightedMorphologicalTree<std::uint8_t>& weighted, NodeId sampleNodeId) {
    ...
    const float expectedResidue =
        altitude[static_cast<std::size_t>(sampleNodeId)] -
        altitude[static_cast<std::size_t>(parentNodeId)];
    requireNear(
        TreeAltitudeAlgorithms::getNodeResidue(tree, view, sampleNodeId),
        expectedResidue,
        1.0e-6f,
    ...
    nonMonotone[static_cast<std::size_t>(parentNodeId)] =
        nonMonotone[static_cast<std::size_t>(sampleNodeId)] + 1.0f;
    requireThrows<std::runtime_error>(
        [&]() {
            TreeAltitudeAlgorithms::validateMonotoneAltitude(tree, std::span<const float>(nonMonotone));
        },
...
void checkUnsignedResidueDoesNotUnderflow(const WeightedMorphologicalTree<std::uint8_t>& weighted, NodeId sampleNodeId) {
    ...
    requireEqual(
        weightedView.getNodeResidue(sampleNodeId),
        expectedResidue,
...
void checkFloatHigraExport(const WeightedMorphologicalTree<std::uint8_t>& weighted) {
    ...
    const auto [parent, exportedAltitude] =
        TreeAltitudeAlgorithms::exportHigraHierarchy(tree, std::span<const float>(altitude));
    ...
    requireEqual(
        ...
        tree.getNumTotalProperParts() + tree.getNumNodes(),
        "templated Higra export compact domain size");
    ...
    for (NodeId leafId = 0; leafId < numLeaves; ++leafId) {
        ...
        require(
            ownerHigraId >= numLeaves && ownerHigraId < static_cast<NodeId>(parent.size()),
            "templated Higra export leaf parent must be an internal Higra node");
        requireNear(
            exportedAltitude[static_cast<std::size_t>(leafId)],
            exportedAltitude[static_cast<std::size_t>(ownerHigraId)],
            1.0e-6f,
...
void checkInt32HigraExport(const WeightedMorphologicalTree<std::uint8_t>& weighted) {
    ...
    const auto [parent, exportedAltitude] =
        TreeAltitudeAlgorithms::exportHigraHierarchy(tree, std::span<const std::int32_t>(altitude));
    ...
    requireEqual(
        static_cast<int>(parent.size()),
        tree.getNumTotalProperParts() + tree.getNumNodes(),
        "int32 templated Higra export compact domain size");
...
template<class T>
void checkGenericAltitudeDelta(const WeightedMorphologicalTree<std::uint8_t>& weighted, const std::string& label) {
    const auto& tree = weighted.topology();
    const std::vector<T> altitude = makeGenericAltitude<T>(weighted);
    const std::span<const T> view(altitude);
    const auto baseline =
    ...
    const auto fromAltitudeSpan =
        detail::computeAscendantsAndDescendantsByAltitude(tree, view, static_cast<AltitudeDiff<T>>(2));
    ...
    requireEqual(
        detail::findAscendantByAltitudeDelta(tree, view, sampleNodeId, static_cast<AltitudeDiff<T>>(2)),
        baseline.first[static_cast<std::size_t>(sampleNodeId)],
        label + " generic findAscendantByAltitudeDelta");
    ...
    requireThrows<std::runtime_error>(
        [&]() {
            const std::vector<T> wrongSize(
                static_cast<std::size_t>(tree.getNumInternalNodeSlots() - 1),
                T{});
            static_cast<void>(detail::computeAscendantsAndDescendantsByAltitude(
                tree,
                std::span<const T>(wrongSize),
    ...
    requireThrows<std::invalid_argument>(
        [&]() {
            static_cast<void>(detail::findAscendantByAltitudeDelta(
                tree,
                view,
                InvalidNode,
                static_cast<AltitudeDiff<T>>(2)));
...
template<class T>
void checkTypedOwnerDeltaAttributeApi(const std::string& label) {
    ...
    auto typedDelta = AttributeComputation::computeSingleAttributeWithDelta(
        typed,
        AREA,
        deltaStep,
        2,
        "last-padding");
    auto canonicalDelta = AttributeComputation::computeSingleAttributeWithDelta(
        canonical,
        AREA,
        AltitudeDiff<std::uint8_t>{1},
        2,
        "last-padding");
    requireDeltaAttributeNear(
        typedDelta,
        canonicalDelta,
        typed.topology(),
        AREA,
        2,
        label + " typed delta AREA must match equivalent canonical layout");

    auto typedLevelDelta = AttributeComputation::computeSingleAttributeWithDelta(
        typed,
        LEVEL,
        deltaStep,
        1,
    ...
    const auto ascendantAndDescendant = detail::computeAscendantsAndDescendantsByAltitude(
        typed.topology(),
        typed.altitudeSpan(),
        deltaStep);
    ...
    for (NodeId nodeId : typed.topology().getAliveNodeIds()) {
        requireNear(
            typedLevelDelta.second[typedLevelDelta.first.linearIndex(nodeId, LEVEL, 0)],
            static_cast<float>(typed.getAltitude(nodeId)),
            1.0e-5f,
        ...
        if (ascendant != InvalidNode && ascendant != nodeId) {
            requireNear(
                typedLevelDelta.second[typedLevelDelta.first.linearIndex(nodeId, LEVEL, -1)],
                static_cast<float>(typed.getAltitude(ascendant)),
                1.0e-5f,
        ...
        requireThrows<std::invalid_argument>(
            [&]() {
                static_cast<void>(AttributeComputation::computeSingleAttributeWithDelta(
                    typed,
                    AREA,
                    std::numeric_limits<AltitudeDiff<T>>::quiet_NaN(),
                    1,
                    "last-padding"));
            },
...
template<class T>
void checkGenericVolumeKernel(const WeightedMorphologicalTree<std::uint8_t>& weighted, const std::string& label) {
    const auto& tree = weighted.topology();
    ...
    for (NodeId nodeId : tree.getAliveNodeIds()) {
        requireNear(
            genericBuffer[volumeNames.linearIndex(nodeId, VOLUME)],
            baseline.second[baseline.first.linearIndex(nodeId, VOLUME)],
            1.0e-5f,
            label + " generic VOLUME must match current path");
        requireNear(
            genericBuffer[volumeNames.linearIndex(nodeId, RELATIVE_VOLUME)],
            baseline.second[baseline.first.linearIndex(nodeId, RELATIVE_VOLUME)],
            1.0e-5f,
            label + " generic RELATIVE_VOLUME must match current path");
    }
    ...
    if constexpr (std::is_floating_point_v<T>) {
        ...
        for (NodeId nodeId : tree.getAliveNodeIds()) {
            ...
            requireNear(
                fractionalBuffer[volumeNames.linearIndex(nodeId, VOLUME)],
                baseline.second[baseline.first.linearIndex(nodeId, VOLUME)] + (area * 0.25f),
                1.0e-5f,
                label + " fractional VOLUME must preserve float altitude contribution");
            requireNear(
                fractionalBuffer[volumeNames.linearIndex(nodeId, RELATIVE_VOLUME)],
                baseline.second[baseline.first.linearIndex(nodeId, RELATIVE_VOLUME)],
                1.0e-5f,
...
template<class T>
void checkGenericGrayLevelStatsKernel(const WeightedMorphologicalTree<std::uint8_t>& weighted, const std::string& label) {
    const auto& tree = weighted.topology();
    const auto areaComputed = AttributeComputation::computeSingleAttribute(weighted, AREA);
    const auto volumeComputed = AttributeComputation::computeAttributes(weighted, {VOLUME, RELATIVE_VOLUME});
    const auto baseline = AttributeComputation::computeAttributes(
        weighted,
    ...
    std::vector<float> genericBuffer(
        static_cast<std::size_t>(tree.getNumInternalNodeSlots()) *
            static_cast<std::size_t>(grayNames.NUM_ATTRIBUTES),
        0.0f);
    ...
    for (NodeId nodeId : tree.getAliveNodeIds()) {
        for (Attribute attribute : requested) {
            requireNear(
                genericBuffer[grayNames.linearIndex(nodeId, attribute)],
                baseline.second[baseline.first.linearIndex(nodeId, attribute)],
                1.0e-5f,
    ...
    if constexpr (std::is_floating_point_v<T>) {
        ...
        for (NodeId nodeId : tree.getAliveNodeIds()) {
            requireNear(
                fractionalGrayBuffer[grayNames.linearIndex(nodeId, LEVEL)],
                baseline.second[baseline.first.linearIndex(nodeId, LEVEL)] + 0.25f,
                1.0e-5f,
                label + " fractional LEVEL must preserve float altitude");
            requireNear(
                fractionalGrayBuffer[grayNames.linearIndex(nodeId, MEAN_LEVEL)],
                baseline.second[baseline.first.linearIndex(nodeId, MEAN_LEVEL)] + 0.25f,
                1.0e-5f,
                label + " fractional MEAN_LEVEL must preserve uniform altitude offset");
            requireNear(
                fractionalGrayBuffer[grayNames.linearIndex(nodeId, VARIANCE_LEVEL)],
                baseline.second[baseline.first.linearIndex(nodeId, VARIANCE_LEVEL)],
                1.0e-4f,
                label + " uniform fractional offset must preserve VARIANCE_LEVEL");
            requireNear(
                fractionalGrayBuffer[grayNames.linearIndex(nodeId, GRAY_HEIGHT)],
                baseline.second[baseline.first.linearIndex(nodeId, GRAY_HEIGHT)],
                1.0e-5f,
                label + " uniform fractional offset must preserve GRAY_HEIGHT");
        }
    }
}

template<class T>
void checkAttributePipelineApi(const WeightedMorphologicalTree<std::uint8_t>& weighted, const std::string& label) {
    const auto& tree = weighted.topology();
    const std::vector<AttributeOrGroup> requests{
        AREA,
        VOLUME,
    ...
    const auto fromAltitudeSpan = AttributeComputation::computeAttributesFromAltitudeSpan(
        equivalentView,
        requests);
    ...
    for (NodeId nodeId : tree.getAliveNodeIds()) {
        for (Attribute attribute : expectedAttributes) {
            requireNear(
                fromAltitudeSpan.second[fromAltitudeSpan.first.linearIndex(nodeId, attribute)],
                baseline.second[baseline.first.linearIndex(nodeId, attribute)],
                1.0e-5f,
                label + " attribute pipeline must match current path for " + AttributeNames::toString(attribute));
            requireNear(
                viewOverload.second[viewOverload.first.linearIndex(nodeId, attribute)],
                fromAltitudeSpan.second[fromAltitudeSpan.first.linearIndex(nodeId, attribute)],
                1.0e-5f,
    ...
    for (NodeId nodeId : tree.getAliveNodeIds()) {
        requireNear(
            singleViewOverload.second[singleViewOverload.first.linearIndex(nodeId, LEVEL)],
            baseline.second[baseline.first.linearIndex(nodeId, LEVEL)],
            1.0e-5f,
            label + " altitude-span single view overload must match current LEVEL");
    }

    requireImageNear(
        AttributeComputation::computeAttributeMapping(equivalentView, LEVEL),
        AttributeComputation::computeAttributeMapping(weighted, LEVEL),
        label + " altitude-span view mapping overload must match current LEVEL mapping");

    const auto grayLevelFromAltitudeSpan = AttributeComputation::computeAttributesFromAltitudeSpan(
        equivalentView,
        {AttributeGroup::GRAY_LEVEL});
    for (NodeId nodeId : tree.getAliveNodeIds()) {
        for (Attribute attribute : {VOLUME, RELATIVE_VOLUME, LEVEL, GRAY_HEIGHT, MEAN_LEVEL, VARIANCE_LEVEL}) {
            requireNear(
                grayLevelFromAltitudeSpan.second[grayLevelFromAltitudeSpan.first.linearIndex(nodeId, attribute)],
                baseline.second[baseline.first.linearIndex(nodeId, attribute)],
                1.0e-5f,
    ...
    if constexpr (std::is_floating_point_v<T>) {
        ...
        const auto fractional = AttributeComputation::computeAttributesFromAltitudeSpan(
            fractionalView,
            requests);
        ...
        for (NodeId nodeId : tree.getAliveNodeIds()) {
            const float area = areaComputed.second[areaComputed.first.linearIndex(nodeId, AREA)];
            requireNear(
                fractional.second[fractional.first.linearIndex(nodeId, VOLUME)],
                baseline.second[baseline.first.linearIndex(nodeId, VOLUME)] + (area * 0.25f),
                1.0e-5f,
                label + " altitude-span API fractional VOLUME must preserve altitude contribution");
            requireNear(
                fractional.second[fractional.first.linearIndex(nodeId, RELATIVE_VOLUME)],
                baseline.second[baseline.first.linearIndex(nodeId, RELATIVE_VOLUME)],
                1.0e-5f,
                label + " altitude-span API uniform fractional offset must preserve RELATIVE_VOLUME");
            requireNear(
                fractional.second[fractional.first.linearIndex(nodeId, LEVEL)],
                baseline.second[baseline.first.linearIndex(nodeId, LEVEL)] + 0.25f,
                1.0e-5f,
                label + " altitude-span API fractional LEVEL must preserve altitude");
            requireNear(
                fractional.second[fractional.first.linearIndex(nodeId, MEAN_LEVEL)],
                baseline.second[baseline.first.linearIndex(nodeId, MEAN_LEVEL)] + 0.25f,
                1.0e-5f,
                label + " altitude-span API fractional MEAN_LEVEL must preserve altitude");
            requireNear(
                fractional.second[fractional.first.linearIndex(nodeId, VARIANCE_LEVEL)],
                baseline.second[baseline.first.linearIndex(nodeId, VARIANCE_LEVEL)],
                1.0e-4f,
                label + " altitude-span API uniform fractional offset must preserve VARIANCE_LEVEL");
            requireNear(
                fractional.second[fractional.first.linearIndex(nodeId, GRAY_HEIGHT)],
                baseline.second[baseline.first.linearIndex(nodeId, GRAY_HEIGHT)],
                1.0e-5f,
                label + " altitude-span API uniform fractional offset must preserve GRAY_HEIGHT");
            requireNear(
                fractional.second[fractional.first.linearIndex(nodeId, MAX_DIST)],
                baseline.second[baseline.first.linearIndex(nodeId, MAX_DIST)],
                1.0e-5f,
                label + " altitude-span API uniform fractional offset must preserve MAX_DIST");
        }
    }
}

template<class T>
void checkTopologyOnlyAttributePipelineApi(const WeightedMorphologicalTree<std::uint8_t>& weighted, const std::string& label) {
    const auto& tree = weighted.topology();
    const std::vector<T> equivalentAltitude = makeEquivalentAltitude<T>(weighted);
    const WeightedTreeView<T> equivalentView(tree, std::span<const T>(equivalentAltitude));
    ...
    requireComputedAttributesEquivalent(
        singleBoundaryFromAltitudeSpan,
        singleBoundaryBaseline,
        tree,
        ATTRIBUTE_GROUPS.at(AttributeGroup::BOUNDARY),
        label + " attribute-pipeline single group overload must support BOUNDARY");

    requireImageNear(
        AttributeComputation::computeAttributeMapping(equivalentView, CONTOUR_PERIMETER),
...
void checkAttributePipelineMaxDistAndRejectsInvalidInputs(const WeightedMorphologicalTree<std::uint8_t>& weighted) {
    ...
    const auto maxDistFromAltitudeSpan = AttributeComputation::computeAttributesFromAltitudeSpan(
        view,
        {MAX_DIST});
    requireComputedAttributesNear(
        maxDistFromAltitudeSpan,
        baselineMaxDist,
        tree,
        {MAX_DIST},
        "altitude-span attribute API must support MAX_DIST");
    ...
    const auto maxDistFromInt32AltitudeSpan = AttributeComputation::computeAttributesFromAltitudeSpan(
        int32View,
        {MAX_DIST});
    requireComputedAttributesNear(
        maxDistFromInt32AltitudeSpan,
        baselineMaxDist,
        tree,
        {MAX_DIST},
    ...
    requireThrows<std::runtime_error>(
        [&]() {
            const std::vector<float> wrongSize(
                static_cast<std::size_t>(tree.getNumInternalNodeSlots() - 1),
                0.0f);
            static_cast<void>(WeightedTreeView<float>(
                tree,
                std::span<const float>(wrongSize)));
        },
...
int main() {
    ...
    require(
        &ownedView.topology() == &weighted->topology(),
        "WeightedMorphologicalTree<std::uint8_t>::asView must borrow the owned topology");
    requireEqual(
        static_cast<int>(ownedView.altitude().size()),
