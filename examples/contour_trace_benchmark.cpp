...
std::size_t storageChecksum(const ContourTraceComputation::IncrementalContourTraces::StorageStats& stats) {
    return stats.addDeltaValues + stats.removeDeltaValues + stats.cachedEdgeValues +
           stats.cachedLoopEdges + stats.cachedLoops + stats.cachedEdgeReadyNodes +
...
void printStorageStats(
    ...
    std::cout << "    " << label << ": "
              ...
              << " cached_edge_values=" << stats.cachedEdgeValues
              << " cached_loop_edges=" << stats.cachedLoopEdges
              << " cached_loops=" << stats.cachedLoops
              << " cached_edge_ready_nodes=" << stats.cachedEdgeReadyNodes
              << " cached_loop_ready_nodes=" << stats.cachedLoopReadyNodes
              << " dense_outgoing_slots=" << stats.traceDenseOutgoingSlots
              << " sparse_outgoing_slots=" << stats.traceSparseOutgoingSlots
...
void measureCase(const std::string& label, const MorphologicalTree& tree, int repeats) {
    ...
    auto [extractOnly, extractMs] = timed([&]() {
    ...
    auto [rootEdgeTraces, rootEdgeExtractMs] = timed([&]() {
        return ContourTraceComputation::extract(tree);
    });
    ...
    std::cout << "  root getEdges(): extract=" << rootEdgeExtractMs
              << " ms materialize=" << rootEdgeMs;
    ...
    auto [rootLoopTraces, rootLoopExtractMs] = timed([&]() {
        return ContourTraceComputation::extract(tree);
    });
    ...
    std::cout << "  root getLoops(): extract=" << rootLoopExtractMs
              << " ms trace=" << rootLoopMs;
    ...
    auto [orderedEdgeTraces, orderedEdgeExtractMs] = timed([&]() {
        return ContourTraceComputation::extract(tree);
    });
    ...
    std::cout << "  all getEdges() tree order: extract=" << orderedEdgeExtractMs
              << " ms materialize=" << orderedEdgeMs;
    ...
    auto [randomLoopTraces, randomLoopExtractMs] = timed([&]() {
        return ContourTraceComputation::extract(tree);
    });
    ...
    std::cout << "  all getLoops() random order: extract=" << randomLoopExtractMs
              << " ms trace=" << randomLoopMs;
    ...
    for (int i = 0; i < repeats; ++i) {
        auto [traces, runExtractMs] = timed([&]() {
            return ContourTraceComputation::extract(tree);
        });
        totalExtract += runExtractMs;
        totalMaterialize += timedVoid([&]() {
    ...
    std::cout << "  repeated materializeAll(): extract_avg=" << totalExtract / repeats
              << " ms materialize_avg=" << totalMaterialize / repeats
              << " ms";
#ifdef MMCFILTERS_ENABLE_CONTOUR_TRACE_PROFILE
