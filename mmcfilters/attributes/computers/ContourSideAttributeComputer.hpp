...
#include "detail/ContourSideFiniteWindowComputation.hpp"
 ...
 *
 * @details
 * This computer projects scalar attributes from its finite-window computation.
 * The public attribute pipeline/topology backend invokes it for `CONTOUR_*`
 * requests.
 *
 * The finite-window computation is performed once and the following attributes
...
class ContourSideAttributeComputer {
  public:
     ...
     */
    inline static constexpr std::array<Attribute, 6> producedAttributes{ContourPixels,    ContourPerimeter, ContourSideNorth,
     ...
     * `context.attrNames`. The method computes the finite-window contour-side
  ...
  public:
    ...
    template <std::floating_point Real> static void computeUnitRows(const UnitAttributeComputeContext<Real>& context) {
        detail::ContourSideAttributeMaterialization::materializeUnitContourSideAttributes(context.tree, context.unitPixels, context.buffer,
