#ifndef ATTRIBUTE_COMPUTED_INCREMENTALLY_CT_HPP
#define ATTRIBUTE_COMPUTED_INCREMENTALLY_CT_HPP
...
class PathAscendantsAndDescendantsCT {
private:
    ComponentTree* tree;;
    ...
    NodeId getNodeAscendantBasedOnHierarchical(NodeId node, int h) {
        NodeId current = node;
        int step = 0;
        while (step++ < h) {
            current = tree->getParentById(current);
            if (current == -1) return current;
        }
        return current;
    }

    void maxAreaDescendants(NodeId nodeAsc, NodeId nodeDes) {
        if (descendants[nodeAsc] == InvalidNode)
            descendants[nodeAsc] = nodeDes;

        if (tree->getAreaById(descendants[nodeAsc]) < tree->getAreaById(nodeDes))
            descendants[nodeAsc] = nodeDes;
    }
...
public:
    explicit PathAscendantsAndDescendantsCT(ComponentTree* tree) : tree(tree) {}

    void computerAscendantsAndDescendants(int delta) {
        ascendants.resize(tree->getNumNodes(), InvalidNode);
        descendants.resize(tree->getNumNodes(), InvalidNode);

        for (NodeId node: tree->getNodeIds()) {
            NodeId nodeAsc = getNodeAscendantBasedOnHierarchical(node, delta);
    ...
    std::vector<NodeId>& getAscendants() { return ascendants; }
    std::vector<NodeId>& getDescendants() { return descendants; }
};


class ContoursCT; //Forward declaration

class AttributeComputedIncrementallyCT {
public:
    ...
    void computerAttribute(ComponentTree* tree, NodeId root);

	static void computerAttribute(ComponentTree* tree, NodeId root, 
		...
		for(NodeId child: tree->getChildrenById(root)){
			AttributeComputedIncrementallyCT::computerAttribute(tree, child, preProcessing, mergeChildren, postProcessing);
			mergeChildren(root, child);
		}
		postProcessing(root);
	}

	std::shared_ptr<ContoursCT> extractCompactContours(ComponentTree* tree);

	static std::vector<std::unordered_set<int>> extractNonCompactContours(ComponentTree* tree);

    static std::pair<std::shared_ptr<AttributeNames>, std::shared_ptr<float[]>> computeAttributesByComputer(ComponentTree* tree, std::shared_ptr<AttributeComputerCT> comp, const DependencyMap& available = {});
	
	static std::pair<std::shared_ptr<AttributeNames>, std::shared_ptr<float[]>> computeSingleAttribute(ComponentTree* tree, AttributeOrGroup attr, const DependencyMap& availableDeps = {});
	
	static std::pair<std::shared_ptr<AttributeNamesWithDelta>, std::shared_ptr<float[]>> computeSingleAttributeWithDelta(ComponentTree* tree, Attribute attribute, int delta, std::string padding="last-padding", const DependencyMap& availableDeps={});

	static std::pair<std::shared_ptr<AttributeNames>, std::shared_ptr<float[]>> computeAttributes(ComponentTree* tree, const std::vector<AttributeOrGroup>& attributes,const DependencyMap& providedDependencies={});

	static ImageFloatPtr computerAttributeMapping(ComponentTree* tree, Attribute attribute);
...
class ContoursCT{
	...
	public:
	ContoursCT(ComponentTree* tree): tree(tree), contours(tree->getNumNodes()), contoursToRemove(tree->getNumNodes()){}
	...
	std::unordered_set<int> getContour(NodeId nodeSubtree) {
		...
		AttributeComputedIncrementallyCT::computerAttribute(tree, nodeSubtree,
	...
	void visitContours(ComponentTree* tree, std::function<void(NodeId, const std::unordered_set<int>&)> visitor) {
	...
	void visitContoursAndCCs(ComponentTree* tree, std::function<void(NodeId, const std::list<int>&, const std::unordered_set<int>&)> visitor) {
		const int numNodes = tree->getNumNodes();
		...
		AttributeComputedIncrementallyCT::computerAttribute(tree, tree->getRootById(),
			...
			// post-processing
			[this, &contoursByNodes, &CCsByNodes, &visitor, &tree](NodeId node) -> void {
				// --- Contornos ---
				auto& contour = contoursByNodes[node];
				if (!contour) {
					contour = std::make_unique<std::unordered_set<int>>();
	...
	class ContourPostOrderIterator {
		...
		public:
		ContourPostOrderIterator(ComponentTree* tree, NodeId root, ContoursCT* contoursCT) : tree(tree), contoursCT(contoursCT){
	...
	class ContourPostOrderRange {
		...
		public:
		ContourPostOrderRange(ComponentTree* tree, NodeId root, ContoursCT* contoursCT) : root(root), tree(tree), contoursCT(contoursCT) {}
...
#endif // ATTRIBUTE_COMPUTED_INCREMENTALLY_CT_HPP
