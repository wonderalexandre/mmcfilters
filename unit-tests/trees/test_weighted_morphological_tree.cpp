...
void requireCompactHigraHierarchy(const std::vector<NodeId>& parent, const std::vector<std::uint8_t>& altitude, int numProperParts, const std::string& label) {
    ...
    for (NodeId properPartId = 0; properPartId < numProperParts; ++properPartId) {
        ...
        requireEqual(altitude[static_cast<size_t>(properPartId)], altitude[static_cast<size_t>(ownerNodeId)],
...
int main() {
    {
        ...
        requireEqual(static_cast<int>(weighted->getAltitudeBuffer().size()), weighted->topology().getNumInternalNodeSlots(),
        ...
        requireVectorEqual(collectImageValues(reconstruction), collectImageValues(image), "weighted reconstruction must match the original image");

        const auto [higraParent, higraAltitude] = weighted->exportHigraHierarchy();
        requireEqual(static_cast<int>(higraParent.size()), weighted->topology().getNumTotalProperParts() + weighted->topology().getNumNodes(),
                     "weighted Higra export parent size");
        requireEqual(static_cast<int>(higraAltitude.size()), weighted->topology().getNumTotalProperParts() + weighted->topology().getNumNodes(),
                     "weighted Higra export altitude size");
        requireCompactHigraHierarchy(higraParent, higraAltitude, weighted->topology().getNumTotalProperParts(), "weighted Higra export");
        ...
        requireVectorEqual(collectImageValues(roundtrip.reconstructionImage()), collectImageValues(image), "weighted Higra round-trip reconstruction");
        ...
        auto maxDistByExportedHigra = AttributeComputation::projectNodeValuesToExportedHigra(*weighted, maxDistNames, maxDistByNode);
        requireEqual(maxDistByExportedHigra.size(), higraParent.size(), "weighted exported-Higra MAX_DIST projection size");
        auto [maxDistHigraNames, maxDistByImportedHigra] = AttributeComputation::computeSingleAttribute(roundtrip, MAX_DIST, NodeIdSpace::HIGRA);
        requireFloatVectorEqualAllowingNaN(maxDistByExportedHigra, maxDistByImportedHigra, "weighted imported/exported Higra MAX_DIST projection");
        ...
        const auto projectedPair = AttributeComputation::projectNodeValuesToExportedHigra(*weighted, areaAndMaxDistNames, areaAndMaxDist);
        ...
        std::vector<float> unitProjectionNodeValues(
            static_cast<size_t>(weighted->topology().getNumInternalNodeSlots()) * static_cast<size_t>(unitProjectionNames.NUM_ATTRIBUTES), 0.0f);
        ...
        const auto projectedUnitValues = AttributeComputation::projectNodeValuesToExportedHigra(*weighted, unitProjectionNames, unitProjectionNodeValues);
        const NodeId sampleProperPart = 10;
        const NodeId sampleOwner = weighted->topology().getProperPartOwner(sampleProperPart);
        const auto [sampleRow, sampleCol] = ImageUtils::to2D(sampleProperPart, weighted->topology().getNumColsOfGridDomain2D());
        requireEqual(projectedUnitValues[unitProjectionNames.linearIndex(sampleProperPart, LEVEL)], static_cast<float>(weighted->getAltitude(sampleOwner)),
                     "exported-Higra unit LEVEL must use the proper-part altitude");
        requireEqual(projectedUnitValues[unitProjectionNames.linearIndex(sampleProperPart, VOLUME)], static_cast<float>(weighted->getAltitude(sampleOwner)),
                     "exported-Higra unit VOLUME must use one pixel at the proper-part altitude");
        requireEqual(projectedUnitValues[unitProjectionNames.linearIndex(sampleProperPart, BOX_COL_MIN)], static_cast<float>(sampleCol),
        ...
        auto reimported = MorphologicalTreeFactory::createFromHigraParent(std::span<const NodeId>(higraParent), std::span<const std::uint8_t>(higraAltitude), 4,
                                                                          4, MorphologicalTreeKind::MAX_TREE, RegularGridAdjacency2D(4, 4, 1.5));
        requireEqual(reimported.getAltitude(importedSampleNodeId), importedSampleAltitude,
                     "weighted Higra import must repopulate the external altitude buffer");
    }

    {
        auto weighted = makeWeightedComponentTree(makeComponentTreeFixture(), true);
        const AltitudeBuffer<std::uint8_t> equalAltitude(static_cast<size_t>(weighted->topology().getNumInternalNodeSlots()), std::uint8_t{7});
        requireThrows<std::runtime_error>([&]() { weighted->setAltitudeBuffer(equalAltitude); },
...

    {
        ...
        const auto [weightedAsc, weightedDesc] = detail::computeAscendantsAndDescendantsByAltitude(
            weighted->topology(), std::span<const std::uint8_t>(weighted->getAltitudeBuffer()), static_cast<AltitudeDiff<std::uint8_t>>(2));
...

    {
        ...
        weighted->setAltitudeBuffer(AltitudeBuffer<std::uint8_t>{0, 1, 2, 3, 4, 5});
        requireEqual(weighted->getAltitude(5), 5, "weighted setAltitudeBuffer must replace the external buffer");
    }

    {
        auto weighted = makeWeightedComponentTree(makeComponentTreeFixture(), true);
        const auto preservedAltitude = weighted->getAltitude(4);

        weighted->mergeNodeIntoParent(5);
        requireEqual(weighted->getAltitude(4), preservedAltitude, "weighted merge must preserve external altitude values");
    }

    {
        ...
        requireEqual(weighted->getAltitude(4), preservedAltitude, "weighted prune must preserve external altitude values");
    }

    {
        ...
        requireEqual(weighted->getAltitude(4), preservedAltitude, "weighted editor merge must preserve external altitude values");
    }

    {
        ...
        requireEqual(weighted->getAltitude(4), preservedAltitude, "weighted editor prune must preserve external altitude values");
    }

    {
        ...
        const std::uint8_t equalChildAltitude = weighted->getAltitude(weighted->topology().getRoot());
        requireThrows<std::runtime_error>([&]() { weighted->setAltitude(rootChildren.front(), equalChildAltitude); },
                                          "weighted setAltitude must reject a max-tree child equal to its parent");

        auto invalidAltitude = weighted->getAltitudeBuffer();
        invalidAltitude[static_cast<size_t>(rootChildren.front())] = equalChildAltitude;
        requireThrows<std::runtime_error>([&]() { weighted->setAltitudeBuffer(invalidAltitude); },
                                          "weighted setAltitudeBuffer must reject a max-tree child equal to its parent");

        const std::uint8_t originalChildAltitude = weighted->getAltitude(rootChildren.front());
        {
            auto editor = weighted->edit();
            require(editor.canRollback(), "ordinary weighted edits must be recoverable");
            editor.setNodeAltitude(rootChildren.front(), equalChildAltitude);
            requireThrows<std::runtime_error>([&]() { editor.commit(); }, "weighted commit must reject a staged non-strict altitude");
        }
        requireEqual(weighted->getAltitude(rootChildren.front()), originalChildAltitude, "abandoned invalid weighted edit must restore the altitude");
        weighted->validateMonotoneAltitude();
    }
...

    {
        ...
        const std::uint8_t insertedAltitude = static_cast<std::uint8_t>(std::max(weighted->getAltitude(3), weighted->getAltitude(4)) + 1);
        auto editor = weighted->edit();
    ...
    {
        auto weighted = makeWeightedComponentTree(makeComponentTreeFixture(), true);
        const auto originalAltitude = weighted->getAltitudeBuffer();
        const NodeId root = weighted->topology().getRoot();

        {
            auto editor = weighted->edit();
            require(editor.canRollback(), "ordinary weighted editor must advertise rollback");
            editor.setNodeAltitude(root, std::numeric_limits<std::uint8_t>::max());
            const TreeValidationResult result = editor.validateAndCommit();
            require(!result.ok, "recoverable weighted commit must reject invalid altitude order");
        }

        require(!weighted->topology().isEditing(), "weighted editor destructor must close through rollback");
        requireVectorEqual(weighted->getAltitudeBuffer(), originalAltitude, "weighted delta rollback must restore altitude");
        weighted->validateMonotoneAltitude();
    }

    {
        auto weighted = makeWeightedComponentTree(makeComponentTreeFixture(), true);
        const auto originalAltitude = weighted->getAltitudeBuffer();
        const int originalSlots = weighted->topology().getNumInternalNodeSlots();

        auto editor = weighted->edit();
        static_cast<void>(editor.createDetachedNode(std::uint8_t{17}));
        editor.rollback();

        requireEqual(weighted->topology().getNumInternalNodeSlots(), originalSlots, "weighted explicit rollback must restore topology slots");
        requireVectorEqual(weighted->getAltitudeBuffer(), originalAltitude, "weighted explicit rollback must restore altitude size and values");
    }

    {
        ...
        auto mixedPolarity = MorphologicalTreeFactory::createFromNativeTopology(
            std::span<const NodeId>(nodeParent), std::span<const NodeId>(properPartOwner), std::span<const std::uint8_t>(altitude), 2, 1, 2,
            HierarchySemantics{MorphologicalTreeKind::TREE_OF_SHAPES, AltitudeOrder::UNCONSTRAINED, NoAdjacency{}});
...

    {
        ...
        editor.rollback();
        WeightedMorphologicalTree<std::uint8_t> moved(std::move(*weighted));
        moved.validateAltitudeBufferShape();
        moved.validateMonotoneAltitude();
    }
