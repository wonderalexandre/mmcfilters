 ...
 * each `AttributeGroup`.
 */
inline const std::unordered_map<AttributeGroup, std::vector<Attribute>>& ATTRIBUTE_GROUPS =
    attributes::registry::attributeGroups();
...
public:
            ...
            for (std::size_t i = 0; i < attributes.size(); ++i) {
                ...
                if (!inserted) {
                    throw std::invalid_argument(
                        "AttributeNamesWithDelta::create received duplicate attribute " +
                        toString(key));
                }
            }
        }
     ...
     */
    [[nodiscard]] int getIndex(Attribute attr, int delta) const {
     ...
     */
    [[nodiscard]] int getIndex(AttributeKey attrKey) const {
     ...
     */
    [[nodiscard]] int linearIndex(int nodeIndex, Attribute attr, int delta) const {
     ...
     */
    [[nodiscard]] int linearIndex(int nodeIndex, AttributeKey attrKey) const {
     ...
     */
    [[nodiscard]] static std::string toString(AttributeKey attrKey) {
...
public:
     ...
     */
    [[nodiscard]] int getIndex(Attribute attr) const {
     ...
     */
    [[nodiscard]] bool contains(Attribute attr) const noexcept {
        return indexMap.contains(attr);
    }

    /**
     * @brief Returns the per-node offset associated with `attr`.
     */
    [[nodiscard]] int offset(Attribute attr) const {
        return getIndex(attr);
    }

    /**
     * @brief Returns the linear index in the flat buffer for `(node, attr)`.
     */
    [[nodiscard]] int linearIndex(int nodeIndex, Attribute attr) const {
     ...
     */
    [[nodiscard]] static std::string toString(Attribute attr) {
        return std::string(attributes::registry::name(attr));
    }
    ...
    static std::string describe(Attribute attr) {
        return std::string(attributes::registry::description(attr));
    }
     ...
     */
    [[nodiscard]] static std::optional<Attribute> parse(const std::string& str) {
        return attributes::registry::parse(str);
    }
};
