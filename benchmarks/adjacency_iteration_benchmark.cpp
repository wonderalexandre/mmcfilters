...
int main(int argc, char** argv) {
    ...
    const int cols = argc > 2 ? std::atoi(argv[2]) : 512;
    ...
    mmcfilters::RegularGridAdjacency2D adjacency(rows, cols, 1.5);
    const int numPixels = rows * cols;
    const double fullMilliseconds =
        benchmarkTraversal(adjacency, numPixels, repetitions,
                           [](const mmcfilters::RegularGridAdjacency2D& relation, int pixel) -> decltype(auto) { return relation.getNeighborIndices(pixel); });
    const double forwardMilliseconds =
        benchmarkTraversal(adjacency, numPixels, repetitions, [](const mmcfilters::RegularGridAdjacency2D& relation, int pixel) -> decltype(auto) {
